...
from app.contractors import activate_invite, authenticate_contractor_link, create_invite, invite_details, revoke_invite
...
from app.deputy_integration import (
    complete_oauth, execute_operation, permission_hash, prepare_operation, save_config, trial_host_allowed,
)
...
class FakeDeputy:
    def __init__(self, user_id: int = 51, employee_id: int = 17):
        self.user_id, self.employee_id, self.calls, self.rosters = user_id, employee_id, [], {}
    def post(self, url: str, **_kwargs: object):
        self.calls.append(("TOKEN", url)); return Response(200, {"access_token": "ACCESS-SECRET", "refresh_token": "REFRESH-SECRET", "expires_in": 3600})
    def request(self, method: str, url: str, **kwargs: object):
        self.calls.append((method, url))
        if url.endswith("/api/v1/me"):
            return Response(200, {"UserId": self.user_id, "EmployeeId": self.employee_id, "Name": "Trial Manager", "Permissions": ["Can_Roster_Manage"]})
