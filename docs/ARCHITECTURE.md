...
Personal current source rows are decorated and grouped into human workdays. Grouping preserves each distinct source description and provenance in `current_source_notes`; combined `description_lines` are derived only from that current bundle. Recognized timing and vehicle/allocation parsing may consume all correctly paired current notes, while ordinary Race Day prose and the main-note fallback use only source-appropriate production/Travel note lines. Raw Deputy notes retain context/vehicle text and equal-text source rows separately. `shift_changes` remains a separate historical stream rendered only by Change History.
...
## 0.5.5 Operational safety

`app/backup_service.py` uses SQLite's online backup API, independently validates the result with `integrity_check` and `foreign_key_check`, hashes it, writes a safe manifest, and only publishes `manifest.json` after the completed backup directory is ready. Managed backup manifests are tagged `re-deputy-backup-v1`; retention removes only old tagged successful backups and never touches historical/operator directories such as `20260824-165141-nzst`. Runtime persistent inventory is deliberate: the SQLite database is authoritative; `track_maps/` can include authoritative manual overrides; `app_secret.key` and `web_push_vapid_private.pem` are copied only as private recovery material when present, never written to a manifest; WAL/SHM, temp files, browser state, and deployment environment secrets are excluded. `APP_SECRET_KEY` supplied by Compose must be retained separately for disaster recovery.

The existing APScheduler owns the daily backup job (default 03:30 Pacific/Auckland, before the 05:00 sync), with `max_instances=1` and coalescing. Backup attempts are stored in `backup_runs`; the Admin-only Safety & recovery panel shows status and can create the same backup manually. Backups are outside `StaticFiles`, have no download route, and restore is an offline CLI only.

`admin_action_audit` is central metadata for every mutating `/admin` route. Middleware writes a durable `started` row before invoking an Admin handler, so an Admin mutation is not started when auditing is unavailable; it finalizes that same row as completed, rejected, blocked, or failed afterwards. If finalization itself is unavailable, the original row deliberately remains `started` rather than becoming a false success record. Its actor FK uses `ON DELETE SET NULL` and keeps display/account snapshots. `app/admin_audit.py` is the only snapshot/redaction path: credential/PIN/token/session/OAuth/VAPID/encryption-shaped keys are redacted centrally, values are bounded, and raw Deputy payloads are never recorded. Specialist domain history remains authoritative. Critical destructive routes add concise before/after context and are blocked when their required safety backup fails.
...
Deputy vehicle evidence is a separate fact from the row's operational Unit. `interpreted_workdays` accepts an already-recorded normalized `vehicle_label` fact on a production row as defensive compatibility; a CCU1 row may therefore retain both `role_label=CCU1` and `vehicle_label=684`. It does not guess from arbitrary nested raw Deputy fields. The real property behind the unusual 28 August combined entry was not captured, so the scalar `vehicle: "684"` regression example is synthetic and does not establish Deputy's JSON schema. The normal primary representation remains separate compatible Travel/vehicle and production rows. The known Deputy `Rental` Area is vehicle/resource evidence under this same model, not a crew position or vacancy. Before schedule rows are presented or shared-positive assignments are materialised, the read-only effective vehicle projection suppresses an older resource only when the same native observer later recorded a different overlapping vehicle resource for the same non-zero Deputy employee, date, and location. Equal capture state, different observers, missing identity/provenance, and every stored source row/observation remain conservative evidence. The interpreter collects every explicit current vehicle fact for the same canonical person/cohort; a roster-note allocation wins; one current explicit value wins next; multiple distinct current values remain an explicit conflict; only then may preceding Travel evidence apply. Owner-scoped personal facts can fill a matching shared blank but never leak to another identity or overwrite a shared explicit disagreement.
