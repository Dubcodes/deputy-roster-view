...
## Operational Teams And Local Applications

Teams are reusable and many-to-many. Northern Team is the first/default team for this installation, but no membership is inferred from names, roles, or location alone. Optional location mappings may classify future planning drafts and Deputy open-shift context without hiding unclassified source shifts.

Published manual open positions may accept applications from the authenticated user's linked canonical person. Team eligibility and work conflicts are enforced server-side; admin acceptance rebuilds only Re-Deputy manual visibility and history. Deputy remains read-only. The resulting audit events are groundwork for future notifications, not notification delivery itself.
